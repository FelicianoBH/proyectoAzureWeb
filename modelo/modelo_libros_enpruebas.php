...
if (isset($_GET['borrarlibro'])) {
	...
	if($libros==="libros") {
			...
			$table .= '<tr>'; 
			$table .= '<td id="'.$codigo_libro.$fila['ID'].'" >' . $fila['ID'] . '</td>';
			$table .= '<td><label id="'.$autor.$fila['ID'].'"  size="30" style="white-space:nowrap">' . $fila['Autor'] . '</label> </td>';
			...
			$table .= '<td colspan="2"><label id="'.$titulo.$fila['ID'].'" size="40" style="white-space:nowrap">' . $fila['titulo'] . '</label> </td>';
			$table .='<td></td>';

			$table .= '<td id="'.$genero.$fila['ID'].'" >'. $fila['genero'] . '</td>';
