<?php 
	...
	if ($acontar=="si") {
		...
		if ($consultar=="a") {
 				$resultado = mysqli_query($con, "SELECT * FROM extractos WHERE EXT_CUENTA ='$decuenta' AND EXT_FECHA  >= '$desde_fechaconver' AND EXT_ASIENTO >= '$desdeasto'  AND EXT_FECHA <= '$hasta_fechaconver' AND EXT_ASIENTO <='$hastaasto'");
 			}

 		if ($consultar=="s") {
 			$resultado = mysqli_query($con, "SELECT * FROM extractos WHERE EXT_CUENTA='$decuenta' AND EXT_FECHA  >= '$desde_fechaconver' AND EXT_ASIENTO >= '$desdeasto' AND EXT_APUNTE >= '00' AND EXT_FECHA <= '$hasta_fechaconver' AND EXT_ASIENTO <='$hastaasto' AND EXT_CONCILIADO='si'");
 			}
 		if ($consultar=="n") {
 			$resultado = mysqli_query($con, "SELECT * FROM extractos WHERE EXT_CUENTA='$decuenta' AND EXT_FECHA  >= '$desde_fechaconver' AND EXT_ASIENTO >= '$desdeasto' AND EXT_APUNTE >= '00' AND EXT_FECHA <= '$hasta_fechaconver' AND EXT_ASIENTO <='$hastaasto' AND EXT_CONCILIADO='NO'");
 			}
...
/*		$resultado = mysqli_query($con, "SELECT * FROM extractos WHERE EXT_CUENTA='$decuenta' AND EXT_FECHA  >= '$desde_fechaconver' AND EXT_ASIENTO >= '$desdeasto' AND EXT_APUNTE >= '00' AND EXT_FECHA <= '$hasta_fechaconver' AND EXT_ASIENTO <='$hastaasto'"); */
	...
	if($extractos=="si" && $consultar !="a") {
		...
		$hasta_fechaconver=date('Y-m-d', $hasta_fechaconver);
 		...
 		if ($consultar=="s") {
 			$resultado = mysqli_query($con, "SELECT * FROM extractos WHERE EXT_CUENTA='$decuenta' AND EXT_FECHA  >= '$desde_fechaconver' AND EXT_ASIENTO >= '$desdeasto' AND EXT_APUNTE >= '00' AND EXT_FECHA <= '$hasta_fechaconver' AND EXT_ASIENTO <='$hastaasto' AND EXT_CONCILIADO='si' LIMIT  $desde,$tamanio");
 			}
 		if ($consultar=="n") {
 			$resultado = mysqli_query($con, "SELECT * FROM extractos WHERE EXT_CUENTA='$decuenta' AND EXT_FECHA  >= '$desde_fechaconver' AND EXT_ASIENTO >= '$desdeasto' AND EXT_APUNTE >= '00' AND EXT_FECHA <= '$hasta_fechaconver' AND EXT_ASIENTO <='$hastaasto' AND EXT_CONCILIADO='NO' LIMIT  $desde,$tamanio");
 			}
			$table = '<div class = "container">';
	...
	if($extractos=="si" && $consultar=="a") {
		...
		$hasta_fechaconver=date('Y-m-d', $hasta_fechaconver);
 		...
 		$resultado = mysqli_query($con, "SELECT * FROM extractos WHERE EXT_CUENTA='$decuenta'");
			...
			$final.= "__".$fila['EXT_FECHA']."**".$fila['EXT_ASIENTO']."**".$fila['EXT_APUNTE'];

			
			if ($fila['EXT_FECHA'] >= $desdefecha && $fila['EXT_FECHA'] <= $hastafecha &&
				$fila['EXT_ASIENTO'] >= $desdeasto && $fila['EXT_ASIENTO'] <= $hastaasto ) {

						$contador_lineas++;

						if ($contador_lineas >= $desde && $contador_lineas <= $desde+$tamanio)  {
						...
						if ($fila['EXT_DEBE_HABER']=="DEBE") {
								...
								$saldo=$saldo_cuenta+$fila['EXT_IMPORTE_DEBE'];
								

								$table .= '<td id="'.$ext_saldo_anterior.$ext_id.'" align="right">' . number_format($saldo_cuenta, 2, ',','.') . '</td>';
								$table .='<td id="'.$ext_importe_debe.$ext_id.'" align="right" style="font-weight:bold;">' . number_format($fila['EXT_IMPORTE_DEBE'], 2, ',','.') . '</td>';
								$table .='<td id="'.$ext_importe_haber.$ext_id.' "align="right" style="font-weight:bold;">' . number_format(0, 2, ',','.') . '</td>';
								 
								$el_debe_haber=1;
						} else {
								$saldo=$saldo_cuenta-$fila['EXT_IMPORTE_HABER'];
			...
			if ($fila['EXT_DEBE_HABER']=="DEBE") {
					$saldo_cuenta += $fila['EXT_IMPORTE_DEBE'];
					} else {
					$saldo_cuenta -= $fila['EXT_IMPORTE_HABER'];
					}
		...
		$hastra=$desde+$tamanio;
		echo $table." ".$pagina." ".$desde." ".$hastra." ".$contador_lineas;
		mysqli_close($con);
	} 
