<?php 
	...
	if ($acontar=="si") {
		...
		$hasta_fechaconver=date('Y-m-d', $hasta_fechaconver);
 				...
 				$resultado = mysqli_query($con, "SELECT * FROM extractos WHERE EXT_CUENTA >='$decuenta' AND EXT_CUENTA <='$hacuenta' AND EXT_FECHA  >= '$desde_fechaconver' AND EXT_ASIENTO >= '$desdeasto' AND EXT_FECHA <= '$hasta_fechaconver' AND EXT_ASIENTO <='$hastaasto'");
	...
	if($extractos=="si"&& $salida=="x") {
		...
		$tralala= $num_filas.$decuenta." ".$hacuenta." ".$desde_fechaconver." ".$desdeasto." ".$hasta_fechaconver." ".$hastaasto;

 				$resultado = mysqli_query($con, "SELECT * FROM extractos LEFT JOIN cuentas ON EXT_CUENTA = CODIGO_CUENTA WHERE EXT_CUENTA >='$decuenta' AND EXT_CUENTA <='$hacuenta' AND EXT_FECHA  >= '$desde_fechaconver' AND EXT_ASIENTO >= '$desdeasto' AND EXT_FECHA <= '$hasta_fechaconver' AND EXT_ASIENTO <='$hastaasto' ORDER BY EXT_CUENTA, EXT_FECHA, EXT_ASIENTO, EXT_APUNTE LIMIT $desde,$tamanio");
	...
	if ($extractos=="si") {
		...
		$table='<br>';
 		...
 		$resultado = mysqli_query($con, "SELECT * FROM extractos LEFT JOIN cuentas ON EXT_CUENTA = CODIGO_CUENTA WHERE EXT_CUENTA >='$decuenta' AND EXT_CUENTA <='$hacuenta' AND EXT_FECHA  >= '$desde_fechaconver' AND EXT_ASIENTO >= '$desdeasto' AND EXT_FECHA <= '$hasta_fechaconver' AND EXT_ASIENTO <='$hastaasto' ORDER BY EXT_CUENTA, EXT_FECHA, EXT_ASIENTO, EXT_APUNTE ASC");
 		...
 		if ($resultado) {
				 		...
				 		while ($fila = mysqli_fetch_assoc($resultado)) {
												...
												if ($sw_inicio) {	
													...
													$table.='<br>';
													$table.= '<div class = "container" style="margin:0px auto;padding:15px;">';
													$table.='<h3 class="salta_pagina" style="text-align:center;"> EXTRACTOS CONTABLES</h3><br>';
													$table.='<input type="text" id="cuenta" size="12" maxlength="12" readonly value="'.$fila['EXT_CUENTA'].'">'; 

													$table.='<input type="text" id="tit" size="3" readonly readonly value="">';

													$table.='<input type="text" id="titulo" size="40" maxlength="40" readonly value="'.$fila['TITULO'].'">';
													...
													$sw_inicio=false;
													$cuenta_registros=5;
													$g_cuenta=$fila['EXT_CUENTA'];
													$g_titulo=$fila['TITULO'];

												} 

												if ($g_cuenta!=$fila['EXT_CUENTA'] || $cuenta_registros>55) {
													...
													$table.='<br>';
													$table.= '<div class = "container" style="margin:0px auto;padding:15px;">';
													$table.='<h3 class="salta_pagina" style="text-align:center;page-break-before: always;"> EXTRACTOS CONTABLES</h3><br>';
													$table.='<input type="text" id="cuenta" size="12" maxlength="12" readonly value="'.$fila['EXT_CUENTA'].'">'; 
													$table.='<input type="text" id="tit" size="3" readonly readonly value="">';
													$table.='<input type="text" id="titulo" size="40" maxlength="40" readonly value="'.$fila['TITULO'].'">';
													$table.= '</div>';
										...
										$table .= '<td NOWRAP id="'.$ext_concepto.$ext_id.'">' . $fila['EXT_CONCEPTO'] . '</td>';
										$table .= '<td id="'.$ext_debe_o_haber.$ext_id.'">' . $fila['EXT_DEBE_HABER'] . '</td>';
									//	$table .= '<td id="'.$ext_saldo_anterior.$ext_id.'">' . $fila['EXT_SALDO_ANTERIOR'] . '</td>';
										...
										if ($fila['EXT_DEBE_HABER']=="DEBE") {
											...
											$table .='<td id="'.$ext_importe_debe.$ext_id.'" align="right" style="font-weight:bold;">' . number_format($fila['EXT_IMPORTE_DEBE'], 2, ',','.') . '</td>';
											$table .='<td id="'.$ext_importe_haber.$ext_id.' "align="right" style="font-weight:bold;">' . number_format(0, 2, ',','.') . '</td>';
											$table .='<td id="'.$saldo_resultante.$ext_id.' "align="right" style="font-weight:bold;">' . number_format($saldo, 2, ',','.') . '</td>';
											$el_debe_haber=1;

											} 

										if ($fila['EXT_DEBE_HABER']=="HABER") {
												$saldo=$fila['EXT_SALDO_ANTERIOR']-$fila['EXT_IMPORTE_HABER'];
