<?php 
	...
	if (isset($_GET['libros'])) {
		...
		$sw_solo=$_GET['sw_solo'];
		$depropietario=$_GET['propietario'];
		}
...
if (isset($_GET['borrarlibro'])) {
	...
	if ($acontar=="si" && $sw_solo=="si") {

		$resultado = mysqli_query($con, "SELECT * FROM libros WHERE propietario = '$depropietario'");
		$num_filas = mysqli_num_rows($resultado);
		echo $num_filas;
	...
	if($libros=="libros") {
			...
			if ($sw_solo=="no") {
		 			$resultado = mysqli_query($con, "SELECT * FROM libros ORDER BY $clasifica LIMIT $desde,$tamanio");
		 					...
		 					$resultado = mysqli_query($con, "SELECT * FROM libros WHERE propietario = '$depropietario' ORDER BY $clasifica LIMIT $desde,$tamanio");
		 				}

			$table = '<div class = "container">';
			$table .=  '<table class = "table table-striped table-bordered table-hover">';
			$table .= '<tr>';
			$table .= '<th class="col-sm-1">Libro</th>';
			...
			$table .= '<td id="'.$autor.$fila['ID'].'" style="white-space:nowrap">' . $fila['Autor'] . ' </td>';

			$table .= '<td id="'.$editorial.$fila['ID'].'" style="white-space:nowrap" >' . $fila['Editorial'] . '</td>';

			$table .= '<td  id="'.$isbn.$fila['ID'].'" style="white-space:nowrap">' . $fila['isbn'] . '</td>';
 
			$table .= '<td id="'.$ubicacion.$fila['ID'].'" style="white-space:nowrap" >' . $fila['ubicacion'] . '</td>';

			$table .= '<td id="'.$titulo.$fila['ID'].'" style="white-space:nowrap">' . $fila['titulo'] . '</td>';

			$table .= '<td id="'.$genero.$fila['ID'].'" style="white-space:nowrap" >'. $fila['genero'] . '</td>';
			$table .= '<td id="'.$sub_genero.$fila['ID'].'" style="white-space:nowrap">'. $fila['sub_genero'] . '</td>';

			$table .= '<td id="'.$propietario.$fila['ID'].'">'. $fila['propietario'] .'</td>';
			
			$table .= '<td><input id="'.$fila['ID'].'" onclick="editarLibros(this.id)" type = "button" value ="Edi" class = "btn btn-success"></td>';
			$table .= '<td><input  id="'.$borrar.$fila['ID'].'" onclick="borrarLibro('.$fila['ID'].')"  type = "button" value ="Borr" class = "btn btn-danger"></td>';
		...
		$table.='<select id="solopropiedad">';
		$table.='<option value="TODOS">TODOS</option>';
		$table.='<option value="ABH">ALVARO</option>';
		$table.='<option value="MHR">MARIBEL</option>';
		$table.='<option value="FBH">FELI</option>';
		$table.='<option value="DBH">DANIEL</option>';
		$table.='<option value="FBF">TATO</option>';
		$table.='</select>';
		$table.="<button  class='botones' onclick="."'soloPropietario()'>"."Un propietario"."</button>";
		$table.= "</div>";
		if($sw_solo=="si") {$solo="Mostrando registros sólo de: ".$propietario;
							echo $table.$solo } else
			{echo $table;}
		mysqli_close($con);
	} 
