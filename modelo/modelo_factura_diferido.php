<?php 
	...
	if($leer_ref=="si") {
			...
			$fila = mysqli_fetch_assoc($resultado);
					...
					$datos_ref->numero_factura=$fila['NUM_ALBARAN'];
	...
	if ($muestra_linea=="si") {
		...
		$resultado = mysqli_query($con, "SELECT * FROM pedido_lineas LEFT JOIN sedes ON PLIN_ALMACEN_ORI = SEDE_CODIGO WHERE PLIN_NUMERO_PEDIDO = '$numero_factura' AND PLIN_NUMERO_LINEA = '$numero_linea'");
		...
		if ($resultado) { 
			...
			$fila = mysqli_fetch_assoc($resultado);
					...
					$datos_linea->numero_factura=$fila['PLIN_NUMERO_PEDIDO'];
	...
	if ($acontar_alblineas=="si") {
		...
		$resultado = mysqli_query($con, "SELECT * FROM pedido_lineas WHERE PLIN_NUMERO_PEDIDO = '$numero_factura'");
	...
	if ($alb_lineas=="si") {
			...
			$desde=(($pagina-1)*$tamanio);
		 	...
		 	$resultado = mysqli_query($con, "SELECT  * FROM pedido_lineas WHERE PLIN_NUMERO_PEDIDO = '$numero_factura' ORDER BY PLIN_NUMERO_PEDIDO DESC, PLIN_NUMERO_LINEA DESC LIMIT $desde,$tamanio"); 
...
if ($editar_cabecera=="si") {
	...
	if ($genera_factura_cab=="si") {
		...
		$resultado=mysqli_query($con, "INSERT INTO factura_cab (FAC_TIPO, FAC_NUMERO, FAC_FECHA, FAC_CLIENTE, FAC_ESTADO, FAC_USUARIO, FAC_IMPORTE_NETO, FAC_IMPORTE_IVA, FAC_IMPORTE_DESCUENTO, FAC_IMPORTE_TOTAL, FAC_NUMERO_LINEAS, FAC_ALMACEN_VENTA, FAC_DOMICILIO_ENTREGA, FAC_TLF, FAC_OBSERVACIONES, FAC_TIPO_VENTA) SELECT PED_TIPO, '$nuevo_numero', PED_FECHA, PED_CLIENTE, PED_ESTADO, PED_USUARIO, PED_IMPORTE_NETO, PED_IMPORTE_IVA, PED_IMPORTE_DESCUENTO, PED_IMPORTE_TOTAL, PED_NUMERO_LINEAS, PED_ALMACEN_VENTA, PED_DOMICILIO_ENTREGA, PED_TLF, PED_OBSERVACIONES, PED_TIPO_VENTA FROM pedido_cab WHERE PED_NUMERO='$numero_pedido' ");
	...
	if ($regraba_linea=="si") {
		...
		$resultado=mysqli_query($con, "UPDATE pedido_lineas SET
			...
			PLIN_ALMACEN_ORI = '$act_almacen_ori', PLIN_CODIGO_ART = '$act_articulo', PLIN_DESCRIPCION = '$act_descripcion', PLIN_UNIDADES = '$act_unidades', PLIN_PRECIO = '$act_precio', PLIN_TIPO_IVA = '$act_tipo_iva', PLIN_IVA_POR = '$act_iva_por', PLIN_IMPORTE_IVA = '$act_iva_importe', PLIN_DESCUENTO_POR = '$act_descuento_por', PLIN_DESCUENTO_IMPORTE = '$act_descuento_importe', PLIN_PRECIO_SIN_IVA = '$act_precio_sin_iva', PLIN_IMPORTE_LINEA = '$act_importe_linea' WHERE PLIN_NUMERO_PEDIDO = '$act_numero_factura' AND PLIN_NUMERO_LINEA = '$act_numero_linea'");
	...
	if ($actualiza_cab=="si") {
		...
		$resultado=mysqli_query($con, "	UPDATE pedido_cab SET 
			...
			WHERE PED_NUMERO = '$numero_factura' ");
