<?php 
	...
	if (isset($_GET['acontar_pedidos'])) {
		$acontar_pedidos=$_GET['acontar_pedidos'];
		
	}

	if (isset($_GET['consulta_pedidos'])) {
		$consulta_pedidos=$_GET['consulta_pedidos'];
		$pagina=$_GET['pagina'];
	...
	if (isset($_GET['genera_albaran_lineas'])) {
		$genera_albaran_lineas=$_GET['genera_albaran_lineas'];
		$numero_pedido=$_GET['numero_pedido'];
		$nuevo_numero=$_GET['nuevo_numero'];
	}
	if (isset($_GET['genera_albaran_cab'])) {
		$genera_albaran_cab=$_GET['genera_albaran_cab'];
		$numero_pedido=$_GET['numero_pedido'];
		$nuevo_numero=$_GET['nuevo_numero'];
	}
	...
	if (isset($_GET['graba_ref_numero_alb'])) {

		$graba_ref_numero_alb=$_GET['graba_ref_numero_alb'];
	...
	if ($muestra_linea=="si") {
		...
		$resultado = mysqli_query($con, "SELECT * FROM pedido_lineas LEFT JOIN sedes ON PLIN_ALMACEN_ORI = SEDE_CODIGO WHERE PLIN_NUMERO_PEDIDO = '$numero_albaran' AND PLIN_NUMERO_LINEA = '$numero_linea'");
		...
		if ($resultado) { 
			...
			$fila = mysqli_fetch_assoc($resultado);
					...
					$datos_linea->numero_albaran=$fila['PLIN_NUMERO_PEDIDO'];
	...
	if ($consulta_pedidos=="si") {
		...
		$table .= '<td><input id="'.$fila['PED_NUMERO'].'" onclick="seleccionaPedido(this.id)" type = "button" value ="Seleccionar" class = "btn btn-success"></td>';
		...
		$table.="<button class='botones' onclick="."'retrocedoPaginaPedidos()'>"."<"."</button>";
		$eligepagina="1-".$num_paginas;
		$table.=' Pagina: <input type="text" id="eligepagina" size="3" maxlength="3" placeholder="'.$eligepagina.'">';
		$table.="<button class='botones' onclick="."'iraPaginaPedidos()'>"."Ir"."</button>";
		$table.="<button  class='botones' onclick="."'avanzoPaginaPedidos()'>".">"."</button>";
		$table.= "</div>";
		$table.= "</div>";
		echo $table;
		mysqli_close($con);
	...
	if ($alb_lineas=="si") {
			...
			$desde=(($pagina-1)*$tamanio);
		 	...
		 	$resultado = mysqli_query($con, "SELECT  * FROM pedido_lineas WHERE PLIN_NUMERO_PEDIDO = '$numero_albaran' ORDER BY PLIN_NUMERO_PEDIDO DESC, PLIN_NUMERO_LINEA DESC LIMIT $desde,$tamanio"); 
			...
			if ($resultado) { 
				...
				$table.= '</table>';
				$table.= '<button onclick="agregarLineas()" class="btn btn-primary">AGREGAR LINEA AL PEDIDO</button>';
				$table.= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				$table.= '<button onclick="generarElAlbaran()" class="btn btn-success">GENERAR ALBARAN </button>';
				$table.="<br><br>";
...
if ($editar_cabecera=="si") {
	...
	if ($borrar_linea=="si") {
			...
			$resultado = mysqli_query($con, "DELETE FROM pedido_lineas WHERE PLIN_NUMERO_PEDIDO ='$num_albaran' AND PLIN_NUMERO_LINEA = '$num_linea'");
			if ($resultado) {
					echo mysqli_affected_rows($con);
				} else { 
						echo mysqli_error($con);
	...
	if ($graba_ref_numero_alb=="si") {

		$id_usuario=$_SESSION["id_usuario"];

		$resultado=mysqli_query($con, "UPDATE referencias SET NUM_ALBARAN =  NUM_ALBARAN+1 WHERE ID_USUARIO= '$id_usuario'");
	...
	if ($genera_historico_pedido_cab=="si") {
		...
		$resultado=mysqli_query($con, "INSERT INTO historico_pedido_cab (HPED_TIPO, HPED_NUMERO, HPED_FECHA, HPED_CLIENTE, HPED_ESTADO, HPED_USUARIO, HPED_IMPORTE_NETO, HPED_IMPORTE_IVA, HPED_IMPORTE_DESCUENTO, HPED_IMPORTE_TOTAL, HPED_NUMERO_LINEAS, HPED_ALMACEN_VENTA, HPED_DOMICILIO_ENTREGA, HPED_TLF, HPED_OBSERVACIONES, HPED_TIPO_VENTA) SELECT PED_TIPO, PED_NUMERO, PED_FECHA, PED_CLIENTE, PED_ESTADO, PED_USUARIO, PED_IMPORTE_NETO, PED_IMPORTE_IVA, PED_IMPORTE_DESCUENTO, PED_IMPORTE_TOTAL, PED_NUMERO_LINEAS, PED_ALMACEN_VENTA, PED_DOMICILIO_ENTREGA, PED_TLF, PED_OBSERVACIONES, PED_TIPO_VENTA FROM pedido_cab WHERE PED_NUMERO='$numero_pedido' ");
	...
	if ($genera_albaran_cab=="si") {

		$resultado=mysqli_query($con, "INSERT INTO albaran_cab (ALB_TIPO, ALB_NUMERO, ALB_FECHA, ALB_CLIENTE, ALB_ESTADO, ALB_USUARIO, ALB_IMPORTE_NETO, ALB_IMPORTE_IVA, ALB_IMPORTE_DESCUENTO, ALB_IMPORTE_TOTAL, ALB_NUMERO_LINEAS, ALB_ALMACEN_VENTA, ALB_DOMICILIO_ENTREGA, ALB_TLF, ALB_OBSERVACIONES, ALB_TIPO_VENTA) SELECT PED_TIPO, '$nuevo_numero', PED_FECHA, PED_CLIENTE, PED_ESTADO, PED_USUARIO, PED_IMPORTE_NETO, PED_IMPORTE_IVA, PED_IMPORTE_DESCUENTO, PED_IMPORTE_TOTAL, PED_NUMERO_LINEAS, PED_ALMACEN_VENTA, PED_DOMICILIO_ENTREGA, PED_TLF, PED_OBSERVACIONES, PED_TIPO_VENTA FROM pedido_cab WHERE PED_NUMERO='$numero_pedido' ");
	...
	if ($genera_albaran_lineas=="si") {

		$resultado=mysqli_query($con, "INSERT INTO albaran_lineas (ALIN_NUMERO_ALBARAN, ALIN_NUMERO_LINEA, ALIN_ALMACEN_ORI, ALIN_CODIGO_ART, ALIN_DESCRIPCION, ALIN_UNIDADES, ALIN_PRECIO, ALIN_TIPO_IVA, ALIN_IVA_POR, ALIN_IMPORTE_IVA, ALIN_DESCUENTO_POR, ALIN_DESCUENTO_IMPORTE, ALIN_PRECIO_SIN_IVA, ALIN_IMPORTE_LINEA, ALIN_TIPO_DOCUMENTO_ORIGEN, ALIN_DOCUMENTO_ORIGEN) SELECT '$nuevo_numero', PLIN_NUMERO_LINEA, PLIN_ALMACEN_ORI, PLIN_CODIGO_ART, PLIN_DESCRIPCION, PLIN_UNIDADES, PLIN_PRECIO, PLIN_TIPO_IVA, PLIN_IVA_POR, PLIN_IMPORTE_IVA, PLIN_DESCUENTO_POR, PLIN_DESCUENTO_IMPORTE, PLIN_PRECIO_SIN_IVA, PLIN_IMPORTE_LINEA, 'PEDIDO', PLIN_NUMERO_PEDIDO  FROM pedido_lineas WHERE PLIN_NUMERO_PEDIDO='$numero_pedido' ");
	...
	if ($grabar_cabecera=="si") {
		...
		$resultado=mysqli_query($con, "INSERT INTO albaran_cab (ALB_TIPO, ALB_NUMERO, ALB_FECHA, ALB_CLIENTE, ALB_ESTADO, ALB_USUARIO, ALB_IMPORTE_NETO, ALB_IMPORTE_IVA, ALB_IMPORTE_DESCUENTO, ALB_IMPORTE_TOTAL, ALB_NUMERO_LINEAS, ALB_ALMACEN_VENTA, ALB_DOMICILIO_ENTREGA, ALB_TLF, ALB_OBSERVACIONES, ALB_TIPO_VENTA) 
			VALUES 
	...
	if ($regraba_linea=="si") {
		...
		$resultado=mysqli_query($con, "UPDATE pedido_lineas SET
			...
			PLIN_ALMACEN_ORI = '$act_almacen_ori', PLIN_CODIGO_ART = '$act_articulo', PLIN_DESCRIPCION = '$act_descripcion', PLIN_UNIDADES = '$act_unidades', PLIN_PRECIO = '$act_precio', PLIN_TIPO_IVA = '$act_tipo_iva', PLIN_IVA_POR = '$act_iva_por', PLIN_IMPORTE_IVA = '$act_iva_importe', PLIN_DESCUENTO_POR = '$act_descuento_por', PLIN_DESCUENTO_IMPORTE = '$act_descuento_importe', PLIN_PRECIO_SIN_IVA = '$act_precio_sin_iva', PLIN_IMPORTE_LINEA = '$act_importe_linea' WHERE PLIN_NUMERO_PEDIDO = '$act_numero_albaran' AND PLIN_NUMERO_LINEA = '$act_numero_linea'");
	...
	if ($actualiza_cab=="si") {
		...
		$resultado=mysqli_query($con, "	UPDATE pedido_cab SET 
			...
			WHERE PED_NUMERO = '$numero_albaran' ");
	...
	if ($opciones=="si") {
		...
		$vista.='<div class="col-md-2">';
					$vista.='<div>';
							$vista.='<button class = "btn btn-success" onclick="xxxxxxx(this.id)">Imprimir</button>';
						$vista.='</div>';		
		$vista.='</div>';
		$vista.='<div class="col-md-2">';
					$vista.='<div>';
