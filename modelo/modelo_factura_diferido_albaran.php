<?php 
	...
	$nuevo_numero="";
	$numero_albaran="";
	$genera_historico_albaran_cab="";
	$genera_historico_albaran_lin="";
	$borra_alb_cab="";
	$borra_alb_lin="";
	$desde_numero_albaran="";
	$consulta_albaranes="";
	$acontar_albaranes="";

	if (isset($_GET['acontar_albaranes'])) {
		$acontar_albaranes=$_GET['acontar_albaranes'];
		
	}

	if (isset($_GET['consulta_albaranes'])) {
		$consulta_albaranes=$_GET['consulta_albaranes'];
		$pagina=$_GET['pagina'];
		$num_paginas=$_GET['num_paginas'];
	}

	if (isset($_GET['borra_alb_cab'])) {
		$borra_alb_cab=$_GET['borra_alb_cab'];
		$numero_albaran=$_GET['numero_albaran'];
	}

	if (isset($_GET['borra_alb_lin'])) {
		$borra_alb_lin=$_GET['borra_alb_lin'];
		$numero_albaran=$_GET['numero_albaran'];
	}

	if (isset($_GET['genera_historico_alb_lin'])) {
		$genera_historico_albaran_lin=$_GET['genera_historico_albaran_lin'];
		$numero_albaran=$_GET['numero_albaran'];
		$nuevo_numero=$_GET['nuevo_numero'];
	}
	if (isset($_GET['genera_historico_albaran_cab'])) {
		$genera_historico_albaran_cab=$_GET['genera_historico_albaran_cab'];
		$numero_albaran=$_GET['numero_albaran'];
		$nuevo_numero=$_GET['nuevo_numero'];
	}
	...
	if (isset($_GET['muestra_linea'])) {
		...
		$numero_factura=$_GET['num_albaran'];
	...
	if (isset($_GET['veo_cab'])) {
	 	...
	 	$numer_albaran=$_GET['numero_factura'];
	 }	
	...
	if ($muestra_linea=="si") {
		...
		$resultado = mysqli_query($con, "SELECT * FROM albaran_lineas LEFT JOIN sedes ON ALIN_ALMACEN_ORI = SEDE_CODIGO WHERE ALIN_NUMERO_ALBARAN = '$numero_factura' AND ALIN_NUMERO_LINEA = '$numero_linea'");
		...
		if ($resultado) { 
			...
			$fila = mysqli_fetch_assoc($resultado);
					...
					$datos_linea->numero_factura=$fila['ALIN_NUMERO_ALBARAN'];
	...
	if ($acontar_alblineas=="si") {
		...
		$resultado = mysqli_query($con, "SELECT * FROM albaran_lineas WHERE ALIN_NUMERO_ALBARAN = '$numero_factura'");
	...
	if ($consulta_albaranes=="si") {
		...
		$table .= '<td><input id="'.$fila['ALB_NUMERO'].'" onclick="seleccionaAlbaran(this.id)" type = "button" value ="Seleccionar" class = "btn btn-success"></td>';
		...
		$table.="<button class='botones' onclick="."'retrocedoPaginaAlbaranes()'>"."<"."</button>";
		$eligepagina="1-".$num_paginas;
		$table.=' Pagina: <input type="text" id="eligepagina" size="3" maxlength="3" placeholder="'.$eligepagina.'">';
		$table.="<button class='botones' onclick="."'iraPaginaAlbaranes()'>"."Ir"."</button>";
		$table.="<button  class='botones' onclick="."'avanzoPaginaAlbaranes()'>".">"."</button>";
		$table.= "</div>";
		$table.= "</div>";
		echo $table;
		mysqli_close($con);
	...
	if ($alb_lineas=="si") {
			...
			$desde=(($pagina-1)*$tamanio);
		 	...
		 	$resultado = mysqli_query($con, "SELECT  * FROM albaran_lineas WHERE ALIN_NUMERO_ALBARAN = '$numero_factura' ORDER BY ALIN_NUMERO_ALBARAN DESC, ALIN_NUMERO_LINEA DESC LIMIT $desde,$tamanio"); 
			...
			if ($resultado) { 
				...
				while ($fila = mysqli_fetch_assoc($resultado)) {
					...
					$table .= '<td id="'.$unidades.$fila['ALIN_NUMERO_ALBARAN'].$fila['ALIN_NUMERO_LINEA'].'" style="white-space:nowrap">' . $fila['ALIN_UNIDADES']. '</td>';
				...
				//	$table .= '<td id="'.$fimporte.$fila['ALIN_NUMERO_ALBARAN'].['ALIN_NUMERO_LINEA'].'">' . $importe_linea . '</td>';
					...
					$table .= '<td id="'.$fdetalle.$fila['ALIN_NUMERO_ALBARAN'].$fila['ALIN_NUMERO_LINEA'].'"><button class = "btn btn-info" onclick="detallesLinea('.$fila['ALIN_NUMERO_LINEA'].')">Detalles</button></td>';
					$table .= '<td id="'.$fdetalle.$fila['ALIN_NUMERO_ALBARAN'].$fila['ALIN_NUMERO_LINEA'].'"><button class = "btn btn-success" onclick="editarLinea('.$fila['ALIN_NUMERO_LINEA'].')">Editar</button></td>';
					$table .= '<td id="'.$fdetalle.$fila['ALIN_NUMERO_ALBARAN'].$fila['ALIN_NUMERO_LINEA'].'"><button class = "btn btn-danger" onclick="borrarLinea('.$fila['ALIN_NUMERO_LINEA'].')">Borrar</button></td>';
		 			$table .= '</tr>';
				}

				$table.= '</table>';
				$table.= '<button onclick="agregarLineas()" class="btn btn-primary">AGREGAR LINEA AL ALBARAN</button>';
				$table.= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
...
if ($editar_cabecera=="si") {
	...
	if ($borra_alb_cab=="si") {
			...
			$resultado = mysqli_query($con, "DELETE FROM albaran_cab WHERE ALB_NUMERO ='$numero_albaran' ");
			if ($resultado) {
	...
	if ($borra_alb_lin=="si") {
			...
			$resultado = mysqli_query($con, "DELETE FROM albaran_lineas WHERE ALIN_NUMERO_ALBARAN ='$numero_albaran' ");
			if ($resultado) {
					echo mysqli_affected_rows($con);
				} else { 
						echo mysqli_error($con);
	...
	if ($sedes==="si") {
		...
		$resultado=mysqli_query($con, "SELECT * FROM albaran_cab WHERE ALB_NUMERO = '$numer_albaran' ");
		...
		if ($resultado) {
			...
			if (mysqli_num_rows($resultado) > 0) {
					...
					$datos_cab->cliente=$fila['ALB_CLIENTE'];
					$datos_cab->estado=$fila['ALB_ESTADO'];
					$datos_cab->fecha=$fila['ALB_FECHA'];
					$datos_cab->importe_neto=$fila['ALB_IMPORTE_NETO'];
					$datos_cab->importe_iva=$fila['ALB_IMPORTE_IVA'];
					$datos_cab->importe_dto=$fila['ALB_IMPORTE_DESCUENTO'];
					$datos_cab->importe_total=$fila['ALB_IMPORTE_TOTAL'];
					$datos_cab->almacen_vta=$fila['ALB_ALMACEN_VENTA'];
					$datos_cab->tlf=$fila['ALB_TLF'];
					$datos_cab->domicilio=$fila['ALB_DOMICILIO_ENTREGA'];
					$datos_cab->observaciones=$fila['ALB_OBSERVACIONES'];
	...
	if ($alta_stock=="si") {
		...
		$resultado=mysqli_query($con, "INSERT INTO stocks (STOCKS_ALMACEN, STOCKS_CODIGO, STOCKS_EXISTENCIAS, STOCKS_MINIMO, STOCKS_PUNTO_ALBARAN, STOCKS_FALTAS, STOCKS_UBI_CALLE, STOCKS_UBI_PASILLO, STOCKS_UBI_NUMERO, STOCKS_UNI_ENTRADA, STOCKS_UNI_SALIDA) VALUES ('$id_almacen_d', '$id_articulo', 0, 0, 0, 0, ' ', ' ', ' ', 0, 0)");
		if ($resultado) {
			echo mysqli_affected_rows($con);
		} else { 
			echo "error".mysqli_error($con);
	...
	if ($genera_historico_albaran_cab=="si") {

		$resultado=mysqli_query($con, "INSERT INTO historico_albaran_cab (HALB_TIPO, HALB_NUMERO, HALB_FECHA, HALB_CLIENTE, HALB_ESTADO, HALB_USUARIO, HALB_IMPORTE_NETO, HALB_IMPORTE_IVA, HALB_IMPORTE_DESCUENTO, HALB_IMPORTE_TOTAL, HALB_NUMERO_LINEAS, HALB_ALMACEN_VENTA, HALB_DOMICILIO_ENTREGA, HALB_TLF, HALB_OBSERVACIONES, HALB_TIPO_VENTA) SELECT ALB_TIPO, ALB_NUMERO, ALB_FECHA, ALB_CLIENTE, ALB_ESTADO, ALB_USUARIO, ALB_IMPORTE_NETO, ALB_IMPORTE_IVA, ALB_IMPORTE_DESCUENTO, ALB_IMPORTE_TOTAL, ALB_NUMERO_LINEAS, ALB_ALMACEN_VENTA, ALB_DOMICILIO_ENTREGA, ALB_TLF, ALB_OBSERVACIONES, ALB_TIPO_VENTA FROM albaran_cab WHERE ALB_NUMERO='$numero_albaran' ");

		if ($resultado) {
				echo mysqli_affected_rows($con)."nuevo numero: ".$nuevo_numero." numero albaran: ".$numero_albaran;
			} else {
				echo "error ".mysqli_error($con);
			}
		mysqli_close($con);
	...
	if ($genera_factura_cab=="si") {
		...
		$resultado=mysqli_query($con, "INSERT INTO factura_cab (FAC_TIPO, FAC_NUMERO, FAC_FECHA, FAC_CLIENTE, FAC_ESTADO, FAC_USUARIO, FAC_IMPORTE_NETO, FAC_IMPORTE_IVA, FAC_IMPORTE_DESCUENTO, FAC_IMPORTE_TOTAL, FAC_NUMERO_LINEAS, FAC_ALMACEN_VENTA, FAC_DOMICILIO_ENTREGA, FAC_TLF, FAC_OBSERVACIONES, FAC_TIPO_VENTA) SELECT ALB_TIPO, '$nuevo_numero', ALB_FECHA, ALB_CLIENTE, ALB_ESTADO, ALB_USUARIO, ALB_IMPORTE_NETO, ALB_IMPORTE_IVA, ALB_IMPORTE_DESCUENTO, ALB_IMPORTE_TOTAL, ALB_NUMERO_LINEAS, ALB_ALMACEN_VENTA, ALB_DOMICILIO_ENTREGA, ALB_TLF, ALB_OBSERVACIONES, ALB_TIPO_VENTA FROM albaran_cab WHERE ALB_NUMERO='$numero_albaran' ");
	...
	if ($genera_historico_albaran_lin=="si") {

		$resultado=mysqli_query($con, "INSERT INTO historico_albaran_lineas (HALIN_NUMERO_ALBARAN, HALIN_NUMERO_LINEA, HALIN_ALMACEN_ORI, HALIN_CODIGO_ART, HALIN_DESCRIPCION, HALIN_UNIDADES, HALIN_PRECIO, HALIN_TIPO_IVA, HALIN_IVA_POR, HALIN_IMPORTE_IVA, HALIN_DESCUENTO_POR, HALIN_DESCUENTO_IMPORTE, HALIN_PRECIO_SIN_IVA, HALIN_IMPORTE_LINEA, HALIN_TIPO_DOCUMENTO_ORIGEN, HALI_DOCUMENTO_ORIGEN) SELECT ALIN_NUMERO_ALBARAN, ALIN_NUMERO_LINEA, ALIN_ALMACEN_ORI, ALIN_CODIGO_ART, ALIN_DESCRIPCION, ALIN_UNIDADES, ALIN_PRECIO, ALIN_TIPO_IVA, ALIN_IVA_POR, ALIN_IMPORTE_IVA, ALIN_DESCUENTO_POR, ALIN_DESCUENTO_IMPORTE, ALIN_PRECIO_SIN_IVA, ALIN_IMPORTE_LINEA, ALIN_TIPO_DOCUMENTO_ORIGEN, ALIN_DOCUMENTO_ORIGEN FROM albaran_lineas WHERE ALIN_NUMERO_ALBARAN='$numero_albaran' ");
	...
	if ($actualiza_cab=="si") {
		...
		$resultado=mysqli_query($con, "	UPDATE albaran_cab SET 
			...
			WHERE ALB_NUMERO = '$numero_factura' ");
