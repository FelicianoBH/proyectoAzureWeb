...
if (isset($_GET['borrarlibro'])) {
	...
	if($libros==="libros") {
			...
			$table .= '<td><label id="'.$autor.$fila['ID'].'"  size="30">' . $fila['Autor'] . '</label> </td>';

			$table .= '<td id="'.$editorial.$fila['ID'].'" >' . $fila['Editorial'] . '</td>';

			$table .= '<td  id="'.$isbn.$fila['ID'].'">' . $fila['isbn'] . '</td>';

			$table .= '<td id="'.$ubicacion.$fila['ID'].'">' . $fila['ubicacion'] . '</td>';

			$table .= '<td colspan="2"><label id="'.$titulo.$fila['ID'].'" size="40">' . $fila['titulo'] . '</label> </td>';
			$table .='<td></td>';
