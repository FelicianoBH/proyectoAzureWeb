...
<div id="wrapper"> 
	<div id="crud_gral"></div>
	  ...
	  <div id="contengo" style="margin:0px auto;">
	  	...
	  	<form>
      		<fieldset>
      			<div id="div1">
					...
					<input type="text" id="codcuenta" size="12" maxlength="12" onfocus="colorTexto(this.id)" onblur="noFoco(this.id)"/>
					<button type="button" class='botones' onclick="buscaCuenta()">Buscar</button>
      			</div>
				<div id="div2">
					<label for="titulo">Titulo: </label>    
					<input type="text" id="titulo" size="30" maxlength="40" readonly="readonly"/>
      			</div>
      			<div id="div3">
      				<label>              </label>
					
					<button type="button" class='botones' onclick="ejecutarNuevaVentana('noedicion','CODIGO_CUENTA')"><u>C</u>onsultar </button>
					<button type="button" class='botones' onclick="ejecutarNuevaVentana('noedicion','TITULO')">Alfabetica</button>
				</div>
      		</fieldset>
      		<fieldset>
      			<div id="div4">
  		...
  		<table>
      			<tr>
      				<td><button type="button" id ="todosmovimientos" class='botones' onclick="todosMovimientos()">TODOS LOS MOVIMIENTOS</button></td>
      				<td><button type="button" id="conciliados" class='botones' onclick="consultaConciliados()">Conciliado</button></td>
					<td><button type="button" id="noconciliados" class='botones' onclick="consultaNoConciliados()"><u>Sin conciliar</button></td>
      				<td><button type="button" id="ambos" class='botones' onclick="consultaAmbos()">Ambos</button></td>
      			</tr>
	 </table>
...
<script type="text/javascript">
	...
	function todosMovimientos(){
		...
		document.getElementById("todosmovimientos").style.backgroundColor='#679B9B';
		...
		consultar="a";	
		document.getElementById("desdefecha").value="0001-01-01";
	...
	function preparaExtractos(){
			...
			if (consultar=="") { consultar="a";}
			decuenta=document.getElementById("codcuenta").value;
		...
		xmlhttp.open("GET", "../modelo/modelo_consulta_extractos.php?acontar="+"si"+"&decuenta="+decuenta+"&desdefecha="+desdefecha+"&desdeasto="+desdeasto+"&hastafecha="+hastafecha+"&hastaasto="+hastaasto+"&conciliados="+consultar, true);
		xmlhttp.send();
	}
	...
	function mostrarExtractos(pagina){
		...
		xmlhttp.open("GET", "../modelo/modelo_consulta_extractos.php?extractos="+"si"+"&pagina="+pagina+"&num_paginas="+num_paginas+"&decuenta="+decuenta+"&desdefecha="+desdefecha+"&desdeasto="+desdeasto+"&hastafecha="+hastafecha+"&hastaasto="+hastaasto+"&conciliados="+consultar, true);
		xmlhttp.send();
	}
	...
	function despejaScreen(){
		...
		xmlhttp.open("GET", "../modelo/modelo_consulta_extractos.php?inicio="+"si", true);
		xmlhttp.send();
	}
...
$('body').on("keydown", function(e) { 
	...
	function consultaConciliados(){
			...
			consultar="s";
			document.getElementById("todosmovimientos").style.backgroundColor='#AACFCF';
	...
	function conciliarExtracto(cuenta_reg) {
		...
		xmlhttp.open("GET", "../modelo/modelo_consulta_extractos.php?concilio="+"si"+"&cta="+cta+"&fec="+fec+"&asto="+asto+"&apu="+apu, true);
		xmlhttp.send();
	}
	function desconciliarExtracto(cuenta_reg) {
		...
		xmlhttp.open("GET", "../modelo/modelo_consulta_extractos.php?concilio="+"no"+"&cta="+cta+"&fec="+fec+"&asto="+asto+"&apu="+apu, true);
		xmlhttp.send();
	}
