...
<div id="nuevaVentana">
			...
			<div id="box-header">Añadir Sub_Familia</div>
			...
			<div id="divac1">
				<p>
				<label for="codigo">Código Sub_Familia : </label>
		    ...
		    <div id="divac2"> 
		     	<p>
			    <label for="nombresubfam"> Nombre Sub_Familia: </label>
			     <input type="text" id="nombresubfam" size="40" maxlength="40"  style="width:350px;height:35px;font-size:12px;"  /></p>
		    </div>
		     <div id="divac22"> 
		     	<p>
			    <label for="nombrefam">Nombre Familia : </label>
			     <input type="text" id="nombrefam" size="40" maxlength="40" readonly="readonly"  style="width:350px;height:35px;font-size:12px;"  /></p>
		    </div>
		     <div id="divac3">
		     	<p>
	...
	<button onmousedown="agregarsubFamilia()" style="margin-left:40%;" class="btn btn-success">Añadir Familia</button><br><br>
</div>
...
<script type="text/javascript">
	...
	function mostrarFamilias(pagina){
		...
		xmlhttp.open("GET", "../modelo/modelo_sub_familia.php?familias="+"si"+"&pagina="+pagina+"&num_paginas="+num_paginas+"&desdecodigo="+desdecodigo, true);
		xmlhttp.send();
	}
	...
	function preparoMostrarFamilias(){
		...
		xmlhttp.open("GET", "../modelo/modelo_sub_familia.php?acontar="+"si"+"&desdecodigo="+desdecodigo, true);
		xmlhttp.send();
	}
	...
	function editarsubFamilia(id) {
		...
		actualizando=true;
		var codigoID="SFAM_CODIGO" + id;
		var nombreID="SFAM_NOMBRE"+id;
		var descuento1ID="SFAM_DESC_1"+id;
		var descuento2ID="SFAM_DESC_2"+id;
	...
	function actualizarsubFamilia(id) {
				...
				if (confirma) {
					...
					xmlhttp.open("GET", "../modelo/modelo_sub_familia.php?actualizar_subfam="+"si"+"&codigo="+id+"&nombreActualizado="+nombreActualizado+"&descuento1Actualizado="+descuento1Actualizado+"&descuento2Actualizado="+descuento2Actualizado, true);
		...
		function borrarsubFamilia(id) {
			...
			var respuesta = confirm("Estas seguro de borrar esta Sub_Familia?");
			...
			if (respuesta ===true ) {
				...
				xmlhttp.open("GET", "../modelo/modelo_sub_familia.php?borrar="+"si"+"&subfamilia_borrar="+id,true);
				xmlhttp.send();
	...
	function agregarsubFamilia() {
		...
		var nuevoCodigo=document.getElementById("codigo").value;	
	 	var nuevoNombre=document.getElementById("nombresubfam").value;
	 	...
	 	var valida=confirm("Confirma Alta de Sub_Familia ?");
		...
		var xmlhttp;
 		...
 		xmlhttp.open("GET", "../modelo/modelo_sub_familia.php?alta="+"si"+"&nuevoCodigo="+nuevoCodigo+"&nuevoNombre="+nuevoNombre+"&nuevoDescuento1="+nuevoDescuento1+"&nuevoDescuento2="+nuevoDescuento2, true);
		xmlhttp.send();
	  } 
	...
	function cargaTeclas() {
			...
			$("#codigo").on('keyup', function(e){
				...
				if(keycode==13 || keycode==40)  {
					...
					var id_subfamilia=document.getElementById("codigo").value;
					var familia = id_subfamilia.slice(0,3);
					if (id_subfamilia.length==6) {

								permito_alta=false;
								buscaFamilia(familia);
								buscasubFamilia(id_subfamilia);

								if (existe_familia && !existe_subfamilia) {

											$("#nombresubfam").prop("disabled", false);
											...
											if (!existe_familia) {
											alert("Familia no existe");
											permito_alta=false;
											enfoca("codigo");
											}	
											if (existe_subfamilia) {
											alert("Sub_Familia ya Existe");
											permito_alta=true;
											enfoca("codigo");
											}
										}
	...
	function buscaFamilia(id){
		...
		xmlhttp.onreadystatechange = function () {
			...
			if (this.readyState === 4 && this.status === 200) {
				...
				document.getElementById("nombrefam").value=xmlhttp.responseText;
				
			}
		}
		xmlhttp.open("GET", "../modelo/modelo_sub_familia?buscafamilia="+"si"+"&id="+id, false);
		xmlhttp.send();
	}
	...
	function buscasubFamilia(id){
		...
		xmlhttp.onreadystatechange = function () {
			...
			if (this.readyState === 4 && this.status === 200) {
				...
				document.getElementById("nombresubfam").value=xmlhttp.responseText;
				
			}
		}
		xmlhttp.open("GET", "../modelo/modelo_sub_familia?buscasubfamilia="+"si"+"&id="+id, false);
		xmlhttp.send();
	}
